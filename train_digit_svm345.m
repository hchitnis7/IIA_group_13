...
validClasses = [0 3 4 5];   % <<< ONLY ALLOWED DIGITS
...
% -------------------------------------------------
fprintf('\nTraining SVM classifiers (digits 0,3,4,5 only)...\n');
...
for s = 1:nSlots
    ...
    % Keep only allowed digits
    classMask = ismember(Ys, validClasses);
    Xs = Xs(classMask,:);
    Ys = Ys(classMask);
...
% Save trained model
% -------------------------------------------------
save('digit_svm_model_0345_ppup.mat','svmModels');
